...
<div class="sf-container" style="max-width: 700px; padding-top: 3rem; padding-bottom: 3rem; text-align: center;">
    ...
    <div style="background: #EDE5D0; border: 1px solid #D8CCAD; padding: 2rem; text-align: left; margin-bottom: 3rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding-bottom: 0.75rem; border-bottom: 1px solid rgba(216,204,173,0.6);">
            <span style="font-size: 0.72rem; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; color: rgba(30,58,42,0.5);">Fulfillment</span>
            <span style="font-size: 0.88rem; font-weight: 600; color: #1E3A2A;">{{ ($order->fulfillment_method ?? 'delivery') === 'pickup' ? 'In-store pickup' : 'Delivery' }}</span>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.6rem 0; border-bottom: 1px solid rgba(216,204,173,0.5);">
            ...
            <span style="font-family: 'Cormorant Garamond', serif; font-size: 1rem; font-weight: 600; color: #1E3A2A;">Nu. {{ number_format($item->line_total_minor / 100) }}</span>
        </div>
        @endforeach
        ...
        <div style="margin-top: 0.75rem; padding: 0.875rem 1.25rem; background: #F7F2E8; border: 1px solid #D8CCAD;">
            <p style="font-size: 0.8rem; color: rgba(30,58,42,0.7); line-height: 1.6;">
                @if($order->isPaid())
                <strong>Payment status:</strong> Paid — your order is being prepared.
                @else
